...
async function bootstrap() {
  ...
  const webDir = webDirCandidates.find((dir) => existsSync(dir));
  if (webDir) {
    app.useStaticAssets(webDir);
    logger.log(`📁 Static files served from: ${webDir}`);
  } else {
    logger.warn('Frontend directory not found. Static assets disabled.');
  }
